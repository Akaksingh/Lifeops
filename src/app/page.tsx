...
export default function Dashboard() {
  ...
  const { create } = useCreateRecord({ client, tableName: TASKS_TABLE });
  ...
  // AI briefing — counts come from the Lemma-backed list.
  ...
  // Sync: extract candidates (server), write the new ones into Lemma (browser).
  async function syncNow() {
    setSyncing(true);
    try {
      const { tasks: extracted } = await fetch('/api/extract').then((r) => r.json());
      const fresh = (extracted ?? []).filter(
        (t: { id: string }) => !existingExtIds.has(t.id)
      );
    ...
    <div className="flex gap-6 px-6 py-6 max-w-[1180px] mx-auto">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="relative mb-5 rounded-2xl bg-gradient-to-br from-purple-100 via-purple-50 to-indigo-100 border border-purple-100 p-5 overflow-hidden">
          ...
          <p className="text-[15px] text-neutral-800 leading-relaxed pl-6 pr-24">
